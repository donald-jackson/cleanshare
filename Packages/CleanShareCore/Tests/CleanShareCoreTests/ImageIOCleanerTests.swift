...
final class ImageIOCleanerTests: XCTestCase {
    ...
    func testJPEGiPhoneSample() async throws {
        try await cleanAndAudit("iphone_sample", "jpg", kind: .jpeg)
    }

    func testJPEGPixelSample() async throws {
        try await cleanAndAudit("pixel_sample", "jpg", kind: .jpeg)
    }

    func testPNGTransparent() async throws {
        try await cleanAndAudit("transparent", "png", kind: .png)
    }

    func testGIFAnimated() async throws {
        try await cleanAndAudit("animated", "gif", kind: .gif)
    }

    func testJPEGLightroom() async throws {
        try await cleanAndAudit("lightroom", "jpg", kind: .jpeg)
    }
}
